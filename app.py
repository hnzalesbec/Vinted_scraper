...
import json
import os
import subprocess # Pro spouštění a zastavování scraperu
import signal # Pro posílání signálů
import psutil # Pro kontrolu běžících procesů
from datetime import datetime, timezone, timedelta # Přidáno timezone
import time
import sys

# --- Názvy souborů (měly by odpovídat backendu) ---
...
STATUS_FILENAME = "scraper_current_status.txt" # Pro aktuální stav scraperu

# --- Výchozí hodnoty (pro případ, že soubory neexistují) ---
...
def load_json_file(filepath, default_data=None, is_jsonl=False):
    ...
    except (json.JSONDecodeError, IOError) as e: st.error(f"Chyba při načítání {filepath}: {e}"); return default_data
...
def try_remove_stale_pid():
    if os.path.exists(PID_FILENAME):
        try: os.remove(PID_FILENAME); st.toast("Zastaralý/neplatný PID soubor byl odstraněn.")
        except OSError: pass 

def get_scraper_live_status():
    if os.path.exists(STATUS_FILENAME):
        try:
            with open(STATUS_FILENAME, 'r', encoding='utf-8') as f:
                status_line = f.read().strip()
                # Zobrazíme jen zprávu za pomlčkou, pokud obsahuje časovou značku
                if " - " in status_line:
                    return status_line.split(" - ", 1)[1]
                return status_line
        except IOError:
            return "Chyba při čtení statusu."
    # Pokud status soubor neexistuje, ale PID ano, scraper možná právě startuje
    elif is_scraper_running():
        return "Scraper běží (čeká na aktualizaci statusu)..."
...
scraper_is_active = is_scraper_running()
if scraper_is_active:
    st.sidebar.success("✅ Scraper je aktivní.")
    if st.sidebar.button("🔴 Zastavit Scraper", type="primary", use_container_width=True, key="stop_scraper_btn"):
        pid = get_scraper_pid()
        if pid:
            try:
    ...
    st.sidebar.info("❌ Scraper není aktivní.")
    if st.sidebar.button("🟢 Spustit Scraper", use_container_width=True, key="start_scraper_btn"):
        try:
            flags = {}; 
            if os.name == 'nt': flags['creationflags'] = subprocess.CREATE_NO_WINDOW
            else: flags['start_new_session'] = True
            process = subprocess.Popen([sys.executable, "main.py"], **flags) # Použije python interpret, kterým běží Streamlit
            with open(PID_FILENAME, 'w') as f: f.write(str(process.pid))
            st.sidebar.success(f"Scraper spuštěn (PID: {process.pid})."); time.sleep(2); st.rerun()
        except Exception as e: st.sidebar.error(f"Chyba při spouštění: {e}")

st.sidebar.markdown("---")
st.sidebar.subheader("ℹ️ Aktuální Stav Scraperu")
scraper_status_message_display = get_scraper_live_status()
st.sidebar.text_area("Status:", scraper_status_message_display, height=100, disabled=True, key="status_display_area")
if st.sidebar.button("🔄 Obnovit status", key="refresh_status_button_sidebar", use_container_width=True):
    st.rerun()
...
selected_profile_display_name = st.sidebar.selectbox(
    "Vyberte profil:", options=profile_options, index=default_profile_radio_index, key="profile_selector_sidebar_v4"
)
...
with tab_form:
    ...
    with st.form(key=f"profile_form_url_{profile_index_for_form if profile_index_for_form != -1 else 'new_url_profile_v4'}"):
        ...
        st.markdown("---")
        vinted_url_input = st.text_area("Vinted URL pro vyhledávání*", 
                                      value=current_profile_data.get("vinted_url", ""), 
                                      height=100,
                                      placeholder="Vložte sem URL z Vinted s nastavenými filtry (např. https://www.vinted.cz/catalog?search_text=nike&brand_ids[]=53&order=newest_first)")
        ...
        if isinstance(must_have_raw, list) and all(isinstance(item, str) for item in must_have_raw): must_have_simple_str = ", ".join(must_have_raw)
            ...
            st.caption("Detekována pokročilá struktura 'Musí obsahovat'. Editujte přímo v JSON souboru pro komplexní AND/OR logiku (např. `[[\"slovoA\", \"slovoB\"], \"slovoC\"]`). Zde zadejte jednoduchý seznam slov oddělených čárkou pro AND logiku.")
        
        must_have_keywords_ui = st.text_input("Musí obsahovat v názvu (slova odd. čárkou = AND)", 
                                            value=must_have_simple_str, 
                                            help="Pro (A nebo B) A ZÁROVEŇ C, zadejte: [[A, B], C] v user_profiles.json. Zde zadejte jednoduchý seznam.")
        
        exclude_keywords_ui = st.text_input("Nesmí obsahovat v názvu (slova odd. čárkou)", 
                                         value=", ".join(local_filters.get("exclude_keywords", [])))
        keywords_case_sensitive = st.checkbox("Rozlišovat velikost písmen u klíč. slov", 
                                           value=local_filters.get("keywords_case_sensitive", False))
        ...
        if submitted:
            if not new_name.strip(): st.error("Název profilu je povinný!")
                ...
                if must_have_keywords_ui.strip():
                    # Zkusíme nejprve parsovat jako JSON, pokud to vypadá jako seznam seznamů
                    if (must_have_keywords_ui.strip().startswith("[[") and must_have_keywords_ui.strip().endswith("]]")) or \
                       (must_have_keywords_ui.strip().startswith("[") and must_have_keywords_ui.strip().endswith("]") and "," in must_have_keywords_ui and any(sub.strip().startswith("[") for sub in must_have_keywords_ui.split("],"))): # Detekce [[], ""]
                        try:
                            parsed_must_have_final = json.loads(must_have_keywords_ui)
                            # Základní validace struktury
                            if not isinstance(parsed_must_have_final, list): raise json.JSONDecodeError("Není list",must_have_keywords_ui,0)
                            for item in parsed_must_have_final:
                                if not (isinstance(item, str) or (isinstance(item, list) and all(isinstance(s, str) for s in item))):
                                    raise json.JSONDecodeError("Nevalidní prvek v must_have_keywords", str(item),0)
                        except json.JSONDecodeError:
                            st.warning("Formát 'Musí obsahovat' vypadá jako pokročilý, ale není validní JSON. Zpracovávám jako jednoduchý AND seznam slov oddělených čárkou."); 
                            parsed_must_have_final = [kw.strip() for kw in must_have_keywords_ui.split(',') if kw.strip()]
                    else: # Bereme jako jednoduchý AND seznam
                        parsed_must_have_final = [kw.strip() for kw in must_have_keywords_ui.split(',') if kw.strip()]
    ...
    if st.session_state.selected_profile_index is not None:
        delete_button_key_final_v3 = f"delete_btn_url_final_v3_{st.session_state.profiles[st.session_state.selected_profile_index].get('name', st.session_state.selected_profile_index)}"
        if st.button(f"🗑️ Smazat profil '{st.session_state.profiles[st.session_state.selected_profile_index].get('name')}'", type="primary", use_container_width=True, key=delete_button_key_final_v3):
...
with tab_finds:
    ...
    if st.button("🔄 Obnovit nálezy", key="refresh_finds_tab_final_v3"):
        st.session_state.all_finds_cache = load_json_file(NEW_FINDS_FILENAME, default_data=[], is_jsonl=True)
        st.session_state.all_finds_cache.sort(key=sort_finds_key, reverse=True) 
        st.rerun()
    ...
    if not st.session_state.all_finds_cache:
        ...
        search_finds_query = st.text_input("🔍 Hledat v nálezech (v názvu):", key="finds_search_input_tab_final_v3").lower()
        ...
        selected_profile_filter = st.selectbox("Filtrovat podle profilu:", available_profiles_for_finds_filter, key="finds_profile_filter_tab_final_v3")
        ...
        if not items_to_display:
            st.info(f"Pro zadaná kritéria nebyly nalezeny žádné položky.")
        else:
                ...
                with st.container():
                    ...
                    with col_details:
                        ...
                        if vinted_ts_val and vinted_ts_val > 0: 
                            try:
                                dt_object_vinted = datetime.fromtimestamp(vinted_ts_val, tz=timezone.utc)
                                dt_object_local = dt_object_vinted.astimezone(None) 
                                now_local_compare = datetime.now(timezone.utc).astimezone(None)
                                time_ago = now_local_compare - dt_object_local
                            ...
                            if our_ts_iso:
                                try: 
                                    dt_object = datetime.fromisoformat(our_ts_iso.replace("Z", "+00:00"))
                                    dt_object_local = dt_object.astimezone(None)
                                    display_time_str = f"Nalezeno scraperem: {dt_object_local.strftime('%d.%m.%Y %H:%M')}"
                                except: pass
                        st.caption(f"{display_time_str} | [Odkaz na Vinted]({find_item_data.get('url', '#')})")
                    st.markdown("</div>", unsafe_allow_html=True)

# --- ZÁLOŽKA: NASTAVENÍ SCRAPERU ---
with tab_settings:
    ...
    with st.form("scraper_settings_form_final_v3"): 
...
with tab_logs_display:
    ...
    if st.button("🔄 Obnovit Logy", key="refresh_log_tab_button_final_v3"): 
        st.rerun()
    if os.path.exists(SCRAPER_LOG_FILENAME):
        try:
            with open(SCRAPER_LOG_FILENAME, 'r', encoding='utf-8') as f:
                log_lines_content = f.readlines()
            st.text_area("Logy:", "".join(log_lines_content[-100:]), height=400, disabled=True, key="log_display_area_tab_final_v3")
